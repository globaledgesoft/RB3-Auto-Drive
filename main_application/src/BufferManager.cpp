...
int BufferManager::SetupGralloc1Interface()
{
    ...
    {
        TEST_ERR("Can not get Gralloc hardware module\n");
        result = -1;
    }
    ...
    uint32_t isUBWC)
{
    ...
    TEST_INFO("AllocateBuffers, Enter");
    ...
    TEST_INFO("AllocateBuffers, EXIT");
    return result;
}
    ...
    uint32_t           index)
{
    ...
    TEST_ERR("vaddr: %p fd: %d", mBufferinfo[index].vaddr, ((private_handle_t *)(*pAllocatedBuffer))->fd);
    ...
    if (mIonFd <= 0) {
        TEST_ERR("Ion dev open failed %s\n", strerror(errno));
        return NULL;
    }
    ...
    if (rc < 0) {
        TEST_ERR("ION allocation failed %s with rc = %d fd:%d\n", strerror(errno), rc, mIonFd);
        return NULL;
    }
    ...
    if (rc < 0) {
        TEST_ERR("ION map failed %s\n", strerror(errno));
        return NULL;
    }
    TEST_DEG("%s ION FD %d len %d\n", __func__, ion_info_fd.fd, alloc.len);
    if (!mIsMetaBuf) {
...
void BufferManager::FreeAllBuffers()
{
    for (uint32_t i = 0; i < mNumBuffers; i++)
    {
        if (NULL != mBuffers[i])
        {
            ...
            native_handle_close(mBuffers[i]);
            native_handle_delete((mBuffers[i]));
#endif
            mBuffers[i] = NULL;
        }
    }
