    ...
    m_IsInit(0)
{
    ...
    while (omxresult == OMX_ErrorNone) {
        ...
        if (omxresult == OMX_ErrorNone)
            TEST_INFO("SupportComponents[%d]: %s", idx, m_SupportComponents[idx]);
        idx++;
    }
...
OMX_BOOL QCameraOMX_Encoder::CheckColorFormatSupported(OMX_COLOR_FORMATTYPE nColorFormat,
    ...
    while (result == OMX_ErrorNone) {
        ...
        TEST_INFO("Support fmt: %#x", pVideoPortFmt->eColorFormat);
        if (result == OMX_ErrorNone && pVideoPortFmt->eColorFormat == nColorFormat) {
            TEST_INFO("Encoder: Format[0x%x] supported by OMX Encoder", nColorFormat);
            bSupported = OMX_TRUE;
            break;
        }
        index++;
...
OMX_ERRORTYPE QCameraOMX_Encoder::SetPortParams(OMX_U32 ePortIndex,
    ...
    if ((nBufferSize == NULL) || (nBufferCount == NULL)) {
        TEST_ERR("buffer error : NULL point");
        return result;
    }
    ...
    if (result != OMX_ErrorNone) {
        TEST_ERR("OMX_GetParameter failed");
        return result;
    }
    ...
    TEST_INFO("%s:%d bitrate is %u", __func__, __LINE__, sPortDef.format.video.nBitrate);

    OMX_INIT_STRUCT_SIZE(&sPortDef, OMX_PARAM_PORTDEFINITIONTYPE);
    result = OMX_SetParameter(m_OmxHandle,
        OMX_IndexParamPortDefinition, (OMX_PTR)&sPortDef);
    if (result != OMX_ErrorNone) {
        TEST_ERR("OMX_SetParameter failed");
        return result;
    }
    ...
    *nBufferSize = sPortDef.nBufferSize;
    TEST_INFO("BufferCount: %d, BufferSize: %d", *nBufferCount, *nBufferSize);
...
OMX_ERRORTYPE QCameraOMX_Encoder::setConfig(omx_config_t *config, QCameraBufferHolder *holder)
{
    ...
    if (m_Config.codec == OMX_VIDEO_CodingAVC) {
        ...
        //not support yet
        TEST_ERR("Unsupported Type\n");
        return omxresult;
    }
    TEST_INFO("bitrate %u, targetBitrate %u, %sBitrateConstant Mode", m_Config.bitrate, \
    ...
    TEST_INFO("%s:%d controlrate is %d, bitrate is %u", __func__, __LINE__, \
    ...
    if (m_inbufs == NULL || m_outbufs == NULL) {
        TEST_ERR("allocate buffer failed");
        return omxresult;
    }
    ...
    for (uint32_t i = 0; i < m_nInputBufferCount; i++) {
        ...
        if (omxresult != OMX_ErrorNone) {
            TEST_ERR("OMX_AllocateBuffer failed");
            return omxresult;
        }
    }
    ...
    if (omxresult != OMX_ErrorNone) {
        TEST_ERR("OMX_SendCommand failed");
    }
...
void QCameraOMX_Encoder::inFilghtFunc(void *arg)
{
    ...
    while (!isExist) {
        ...
        if (m_infightQ != NULL && m_infightQ->size()) {
            ...
            if (0 != toWait(&m_inCond,&m_inLock,sec_to_wait)) {
                TEST_ERR("%s %d waiting timeout",__func__,__LINE__);
            }
            pthread_mutex_unlock(&m_inLock);
            continue;
        }
        ...
        case READ:
        {
            ...
            if (result != OMX_ErrorNone) {
                TEST_ERR("OMX_EmptyThisBuffer failed");
            }
            break;
        }
        default:
    ...
    TEST_INFO("%s:%d Thread will exit!", __func__, __LINE__);
}
...
void QCameraOMX_Encoder::outFilghtFunc(void *arg)
{
    ...
    while (!isExit) {
        ...
        case WRITE:
        {
            ...
            if (result != OMX_ErrorNone) {
                TEST_ERR("OMX_FillThisBuffer failed");
            }
            break;
        }
        ...
        default:
        {
            TEST_ERR("outFilghtFunc Unsupported type\n");
            break;
        }
        }
        delete q;
...
void QCameraOMX_Encoder::stop(){
    ...
    omxresult = OMX_SendCommand(m_OmxHandle,
        OMX_CommandStateSet, (OMX_U32)OMX_StateIdle, NULL);
    if (omxresult != OMX_ErrorNone) {
        TEST_ERR("%s OMX_SendCommand error\n", __func__);
        return;
    }
    pthread_mutex_lock(&m_stateMutex);
    ...
    for (uint32_t i = 0; i < m_nInputBufferCount; i++) {
        ...
        if (omxresult != OMX_ErrorNone) {
            TEST_ERR("%s OMX_FreeBuffer error\n", __func__);
            return;
        }
    }
...
OMX_ERRORTYPE QCameraOMX_Encoder::enableMetaMode(OMX_U32 portidx)
{
    ...
    if (result != OMX_ErrorNone) {
        TEST_ERR("%s OMX_SetParamrter error\n", __func__);
        return result;
    }
    TEST_INFO("enable meta mode Done");
...
OMX_ERRORTYPE QCameraOMX_Encoder::onOmxEvent(
    ...
    OMX_IN OMX_PTR pEventData)
{
    TEST_INFO("%s:%d, eEvent: %d, nData1: %d, nData2: %d, pEventData: %p", \
    ...
    if (eEvent == OMX_EventCmdComplete) {
        if (nData1 == OMX_CommandStateSet) {
            ...
            pthread_mutex_unlock(&m_stateMutex);
            TEST_INFO("receive event stateset : %d [%s]", nData2, state[nData2]);
        } else if (OMX_CommandPortEnable == nData1) {
            TEST_INFO("PortEnable on: %d", nData2);
        } else if (OMX_CommandPortDisable == nData1) {
            TEST_INFO("PortDisable on: %d", nData2);
        }
    } else if (eEvent == OMX_EventError){
        TEST_ERR("event Error: %d %d", nData1, nData2);
    }
    return OMX_ErrorNone;
}
