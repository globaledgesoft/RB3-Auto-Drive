...
TestConfig::TestConfig()
{
    ...
    memset(&mDepthStream,  0, sizeof(stream_info_t));
...
int TestConfig::parseCommandlineMetaUpdate(char* order,android::CameraMetadata* metaUpdate)
{
    ...
    while (*order != '\0' && !errfnd) {
        ...
        case EXPOSURE_METERING:{
            ...
            ALOGI("======exposure metering[%d]===[0x%x]\n",metering,tag);
            break;
        }
        case SELECT_PRIORITY:{
            ...
            CameraMetadata::getTagFromName("org.codeaurora.qcamera3.iso_exp_priority.use_iso_exp_priority",vTags.get(), &tag1);
	    camera_metadata_entry entry = metaUpdate->find (tag1);

            ALOGI("===pro===exposure meter mthod[%d]==[%d]=[0x%x-0x%x]\n",entry.count,selPriority,tag,tag1);
            break;
        }
        case EXP_PRIORITY:{
            uint32_t tag = 0;
...
int TestConfig::parseCommandlineAdd(int ordersize,char* order)
{
	...
	case SNAPSHOT_SIZE_OPT:{
        ...
        case PREVIEW_FORMAT_OPT:{
            ...
            } else if (!strcmp("raw8", value)) {
                mPreviewStream.format = HAL_PIXEL_FORMAT_Y8;
	...
	case DEPTH_FORMAT_OPT:{
             ...
             *****/
            if (!strcmp("yuv420", value)) {
                mDepthStream.format = HAL_PIXEL_FORMAT_YCBCR_420_888;
            } else if (!strcmp("yuv_ubwc", value)) {
                mDepthStream.format = HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED;
            } else if (!strcmp("raw10", value)) {
                mDepthStream.format = HAL_PIXEL_FORMAT_RAW10;
            } else if (!strcmp("raw8", value)) {
                mDepthStream.format = HAL_PIXEL_FORMAT_Y8;
            } else if (!strcmp("raw16", value)) {
	...
	case SENSOR_ALTEK:{
		int alteksensor = atoi(value);
		printf("%s altek sensor \n", (alteksensor  ? "is" : "not"));
		mAltekSensor = alteksensor > 0 ? true : false;
		break;
	}
        default:
	...
	case ((1 << TESTMODE_PREVIEW)| ( 1<< TESTMODE_TOF)):
            mTestMode = TESTMODE_TOF;
            break;
...
/************************************************************************
 * * name : parseCommandlineChange
 * * function: Update info from cmd for open camera.
 * ************************************************************************/
int TestConfig::parseCommandlineChange(int ordersize,char* order)
{
    enum {
        ID_OPT = 0,
    };
    ...
    while (*order != '\0' && !errfnd) {
        ...
        break;
       default:
           printf("WARNING Command Change unsupport order param: %s \n", value);
           break;
       }
    }
