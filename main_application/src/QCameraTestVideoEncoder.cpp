...
void QCameraTestVideoEncoder::stop()
{
    ...
    while (true) {
        ...
        TEST_ERR("QCameraTestVideoEncoder::stop: mBufferQueue:%d\n",mBufferQueue->size());
...
int QCameraTestVideoEncoder::Read(OMX_BUFFERHEADERTYPE *buf)
{
    ...
    pthread_mutex_unlock(&mLock);
    if (!buf_handle) {
        TEST_ERR("buffer handle is NULL");
        return -1;
    } else {
        ...
        TEST_INFO("meta_buffer->meta_handle %p", meta_buffer->meta_handle);
#endif
    }
